...
const db = new sqlite3.Database('./movies_database.sqlite', (err) => {
  if (err) {
...
db.serialize(() => {
  db.run(
    `CREATE TABLE IF NOT EXISTS movies(id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT , director TEXT , genre TEXT , release_year INTEGER, rating REAL , actor TEXT , box_office_collection REAL)`,
    (err) => {
      if (err) {
  ...
  const stmt =
    db.prepare(`INSERT INTO movies (title , director , genre , release_year , rating , actor , box_office_collection)
            VALUES (?,?,?,?,?,?,?)`);
  ...
  stmt.finalize();
  console.log('Inserted 18 bollywood movies into the movies table');

  //Closing the connection
  db.close((err) => {
    if (err) {
      console.error('Error in closing the database :', err.message);
    } else {
