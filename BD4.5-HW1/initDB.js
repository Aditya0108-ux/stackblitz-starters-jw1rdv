...
const db = new sqlite3.Database('./courses_database.sqlite', (err) => {
  if (err) {
...
db.serialize(() => {
  db.run(
    `CREATE TABLE IF NOT EXISTS courses(id INTEGER PRIMARY KEY AUTOINCREMENT , name TEXT , description TEXT , duration INTEGER , instructor TEXT , rating REAL , price INTEGER)`,
    (err) => {
      if (err) {
  ...
  stmt.finalize();
  console.log('Inserted 15 rows into the database');
