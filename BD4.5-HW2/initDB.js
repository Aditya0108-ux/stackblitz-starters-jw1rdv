...
const db = new sqlite3.Database('./employees_database.sqlite', (err) => {
  if (err) {
...
db.serialize(() => {
  ...
  name TEXT , position TEXT , department TEXT , salary INTEGER , years_of_experience INTEGER)`,
    (err) => {
      if (err) {
  ...
  stmt.finalize();
  console.log('Inserted 5 employees into the Employees table');
