...
app.get('/companies/details/:id', async (req, res) => {
  ...
  try {
    ...
    if (result.company.length === 0) {
      return res
        .status(404)
        .json({ message: 'No companies found for the id' + id });
    }
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ error: error.message });
...
app.get('/companies/industry/:industry', async (req, res) => {
  ...
  try {
    ...
    if (result.companies.length === 0) {
      return res
        .status(404)
        .json({ message: 'No companies found of the industry : ' + industry });
    }
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ error: error.message });
  ...
  try {
    ...
    if (result.companies.length === 0) {
      return res
        .status(404)
        .json({
          message: 'No companies sorted according to the ' + order + ' order',
        });
    }
    return res.status(200).json(result);
  } catch (error) {
